...
class data_generator:
    ...
    def attn_matrix(self, sim_mat, eps=1e-20):
        ...
        if self.shift is not None:
            mask = np.triu(
                mask, k=int(self.shift * self.w + self.window * self.w // 2)
            ) + np.tril(
                mask, k=int(self.shift * self.w - self.window * self.w // 2) - 1
            )
        else:
            self.shift = self.global_translation_on_x()
            mask = np.triu(
                mask, k=int(self.shift + self.window * self.w // 2)
            ) + np.tril(mask, k=int(self.shift - self.window * self.w // 2) - 1)
        mask[:, :left_idx] = 1
        mask[:, right_idx:] = 1
        mask[:top_idx, :] = 1
        mask[bottom_idx:, :] = 1
    ...
    def displacement_fit(self, attn_masked, mask):
        ...
        print(
            f"anchored column: \t\t {np.around(-coef[1] / 2 / (coef[0] + 1e-20), 2)} ({np.around(-coef[1] / 2 / (coef[0] + 1e-20) / self.w * 100, 1)}%)"
        )
        print(
            f"translation: \t\t\t {np.around(coef[2] - (coef[1] ** 2) / 4 / (coef[0] + 1e-20), 2)}"
        )
    ...
    def boundary_mask(self, coef, mask):
        ...
        x_retrace_mask = np.logical_and(
            (x_retrace_idx >= x_retrace_min), (x_retrace_idx <= x_retrace_max)
        )
    ...
    def generate(self):
        ...
        self.coef, self.xx, self.yy, self.ww = self.displacement_fit(
            self.attn_masked, self.mask
        )
        xtm, xrm = self.boundary_mask(
            self.coef, self.mask
        )  # x_trace_mask, x_retrace_mask
    ...
    def plot(self):
        ...
        ax[1].imshow(
            1 - self.mask, "grey", interpolation=None, alpha=0.1, extent=[0, 1, 1, 0]
        )
        ax[1].plot(
            self.x_eval / self.w,
            (self.x_eval + self.y_eval) / self.w,
            color="white",
            linestyle="dashed",
            alpha=0.8,
        )
    ...
    def save(self):
        ...
        np.savetxt(
            os.path.join(self.data_path, "fitting_parameters.csv"),
            self.coef,
            delimiter=",",
        )
        io.imsave(
            os.path.join(self.data_path, "trace.tif"),
            self.imgs1.astype(np.float32),
            check_contrast=False,
        )
        io.imsave(
            os.path.join(self.data_path, "retrace.tif"),
            self.imgs2.astype(np.float32),
            check_contrast=False,
        )

        io.imsave(
            os.path.join(self.data_path, "overlap.tif"),
            np.concatenate(
                ([np.mean(self.imgs1, axis=0)], [np.mean(self.imgs2, axis=0)]), axis=0
            ).astype(np.float32),
            check_contrast=False,
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="subpixel alignment of AFM columns between trace and retrace"
    )
    parser.add_argument(
        "-rp", "--raw_path", type=str, default="raw", help="raw images path"
    )
    parser.add_argument(
        "-tf", "--trace_file", type=str, default="trace.tif", help="trace filename"
    )
    parser.add_argument(
        "-rf",
        "--retrace_file",
    ...
    parser.add_argument(
        "-sf", "--shift", type=float, default=None, help="attention area shift"
    )
    parser.add_argument(
        "-w", "--window", type=float, default=0.1, help="attention window size"
    )
    parser.add_argument(
        "-tl",
        "--trace_left",
        type=float,
        default=0.05,
        help="trace left boundary ratio",
    )
    parser.add_argument(
        "-tr",
        "--trace_right",
        type=float,
        default=0.95,
        help="trace right boundary ratio",
    )
    parser.add_argument(
        "-rl",
        "--retrace_left",
        type=float,
        default=0.0,
        help="retrace left boundary ratio",
    )
    parser.add_argument(
        "-rr",
        "--retrace_right",
        type=float,
        default=1.0,
        help="retrace right boundary ratio",
    )
    parser.add_argument(
        "-t", "--temperature", type=float, default=0.01, help="temperature in attention"
    )
    parser.add_argument(
        "-c", "--coef", nargs="+", type=float, help="predefined coefficients"
    )
