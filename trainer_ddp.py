...
class train_net:
    def __init__(
        ...
        self.checkpoint_save_path = os.path.join(
            self.checkpoint_path, f"ensemble{self.ensemble}"
        )
    ...
    def train(self, rank, world_size):
        ...
        if self.epochs is None:
            self.epochs = round(
                self.iteration / math.ceil(len(dataset) / self.bs / world_size)
            )
            self.epochs = self.epochs if self.epochs else 1

        optimizer = torch.optim.Adam(
            ddp_model.parameters(), lr=self.lr, betas=(self.b1, self.b2)
        )
        ...
        iter_count = 0
        with tqdm(
            range(self.epochs * len(dataloader)),
            desc=f"ensemble{self.ensemble}-{self.mode}",
            disable=(rank != 0),
        ) as pbar:
            for epoch in range(self.epochs):
                ...
                for img_A, img_B in dataloader:
                    ...
                    loss = self.loss_weight * self.MSELoss(img_A_pred, img_B) + (
                        1 - self.loss_weight
                    ) * self.L1Loss(img_A_pred, img_B)
        ...
        if rank == 0:
            torch.save(
                ...
                f"{self.checkpoint_save_path}/{self.mode}_checkpoint_last.pth",
            )
...
if __name__ == "__main__":
    ...
    for i in range(opt.ensembles):
        for mode in ["t2r", "r2t"]:
            mp.spawn(
                train_worker,
                args=(opt.world_size, opt, mode, i),
