...
class pred_net:
    ...
    def predict(self, rank, ensemble_idx):
        ...
        checkpoint_save_path = os.path.join(
            self.checkpoint_path, f"ensemble{ensemble_idx}"
        )
        prediction_save_path = os.path.join(
            self.prediction_path, f"ensemble{ensemble_idx}"
        )
        if not os.path.exists(prediction_save_path):
            os.makedirs(prediction_save_path, exist_ok=True)

        for mode in ["t2r", "r2t"]:
            data_A, data_B, img_shape, mi, ma = load_data(
                self.data_path,
                mode,
                device,
            ...
            self.model.load_state_dict(
                torch.load(
                    os.path.join(
                        ...
                        f"{mode}_checkpoint_{self.checkpoint_idx}.pth",
                    )
                )
            )
            ...
            outputs = stitch_patches(
                outputs, img_shape, self.patch_size, self.overlap_th, sigma=1 / 3
            )
            outputs = outputs * (ma - mi) + mi

            io.imsave(
                f"{prediction_save_path}/{mode}_{self.checkpoint_idx}.tif",
...
if __name__ == "__main__":
    ...
    ensembles_n = len(
        [d for d in os.listdir(opt.checkpoint_path) if d.startswith("ensemble")]
    )
