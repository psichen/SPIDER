...
def get_patch_index(img_size, patch_size, overlap_th=0.2):
    i_beg = [0]
...
def tile_patches(imgs, patch_size=(64, 64), overlap_th=0.2):
    if imgs.ndim == 2:
...
def stitch_patches(
    patches, image_shape, patch_size=(64, 64), overlap_th=0.2, sigma=1 / 3
):
    b, h, w = image_shape
...
def load_data(
    ...
    trace = io.imread(os.path.join(data_path, "trace.tif"))
    retrace = io.imread(os.path.join(data_path, "retrace.tif"))
    if trace.ndim == 2:  # hw
