...
```math
L_{t2r} = \mathbb{E} \Vert f_\theta(T) - R \Vert ^2
```
...
```math
L_{t2r} = \mathbb{E} \Vert f_\theta(T) - R_0 \Vert ^2 + \mathbb{E} \Vert R - R_0 \Vert ^2
```
...
The training script will train two models separately to denoise trace based on retrace (*t2r*) and denoise retrace based on trace (*r2t*), respectively.
